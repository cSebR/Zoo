...
class AnimauxController{
    ...
    public function visualisation(){
        if(Securite::verifAccessSession()){
            ...
            require_once "views/animauxVisualisationView.php";
        }else{
    ...
    public function modification($idAnimal){
        if(Securite::verifAccessSession()){
            ...
            foreach($lignesAnimal as $continent){
                $tabContinent[] = $continent['continent_id'];
            }

            $animal = array_slice($lignesAnimal[0],0,5);
            
            require_once "views/animauxModificationView.php";
        }
        else{
            throw new Exception(("vous n'avez pas le droit"));
        }
    ...
    public function modificationValidation(){
        if(Securite::verifAccessSession()){ 
            ...
            if($_FILES['image']['size'] > 0){
                ...
                $image = ajoutImage($_FILES,$repertoire);
            }
            ...
            foreach ($continents as $key => $continent){
                //continent coché et non présent en BDD
                ...
                //continent non coché et présent en BDD
                if(!$continents[$key] && $continentsManager->verificationExisteAnimalContinent($idAnimal,$key)){
                    $continentsManager->deleteDBAnimalContinent($idAnimal,$key);
                }
            }
            
            $_SESSION['alert'] = [
                "message" => "L'animal est modifié",
                "type" => "alert-success"
            ];
    ...
    public function creation(){
        if(Securite::verifAccessSession()){
            ...
            require_once "views/animauxCreationView.php";
        }else{
            throw new Exception(("vous n'avez pas le droit"));
        }
    ...
    public function creationValidation(){
        if(Securite::verifAccessSession()){
            ...
            $_SESSION['alert'] = [
                "message" => "L'animal est crée avec l'id:".$idAnimal,
                "type" => "alert-success"
             ];
